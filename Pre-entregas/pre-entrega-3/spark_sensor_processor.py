...
parsed_stream = raw_stream \
    ...
    .select("data.*") \
    .withColumn("event_time", to_timestamp(col("timestamp"), "dd-MM-yyyy HH:mm:ss"))

# 5. AGREGACIÓN CON VENTANA TEMPORAL (WINDOWING)
aggregated_metrics = parsed_stream \
    .groupBy(
        window(col("event_time"), "1 minute"), # Ventana fija de 1 minuto basada en la hora del evento
